...
output "aws_lb_dns_name" {
  value = aws_lb.devopinator.dns_name
}

output "aws_lb_zone_id" {
  value = aws_lb.devopinator.zone_id
}


resource "aws_lb" "devopinator" {
  name               = var.lb_name
...
resource "aws_lb_target_group_attachment" "devopinator_target_group_attachment" {
  target_group_arn = var.lb_target_group_arn
  target_id        = var.ec2_instance_id # Replace with your EC2 instance reference
  port             = var.lb_target_group_attachment_port
}

resource "aws_lb_listener" "devopinator_listner" {
  load_balancer_arn = aws_lb.devopinator.arn
...
resource "aws_lb_listener" "devopinator_https_listner" {
  load_balancer_arn = aws_lb.devopinator.arn
